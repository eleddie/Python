def MaxMin(operador):#Calcula el valor maximo o minimo segun la opcion seleccionada
    evaluando = elementos[0]
    for numero in elementos: 
        exec("if evaluando " + operador + " numero:\n    evaluando = numero")#Se ejecuta la cadena como una sentencia con el operador recibido
...
def opcionMenu(opcion):#Recibe como parametro la opcion seleccionada
    if opcion == "1":#Calcular Maximo
        print "\nMaximo:", MaxMin("<")#Se llama a la funcion MaxMin y se pasa el operador "<"
    elif opcion == "2":#Calcular Minimo
        print "\nMinimo:", MaxMin(">")#Se llama a la funcion MaxMin y se pasa el operador ">"
    elif opcion == "3":#Salir
        exit()
    else:#Imprime error
